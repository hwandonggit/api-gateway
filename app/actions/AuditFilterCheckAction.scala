...
/** Action to check json parameters .
  ...
  */
class AuditFilterCheckAction @Inject()(val parser: BodyParsers.Default,
                                       logger: LogWriter,
                                       config: Configuration)
                                      (implicit ec: ExecutionContext)
  extends ActionFilter[AuditFilterRequest] {
  ...
  /** filter component
    ...
    * @return
    */
  override def filter[A](request: AuditFilterRequest[A]) = Future.successful {
    request.filter match {
      case Some(result) => goSuccess
      case None => goFailed
    }
  }
