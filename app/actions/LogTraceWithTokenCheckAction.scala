...
/** Action to check json parameters .
  ...
  */
class LogTraceWithTokenCheckAction @Inject()(val parser: BodyParsers.Default,
                                             logger: LogWriter,
                                             config: Configuration)
                                            (implicit ec: ExecutionContext)
  extends ActionFilter[LogTraceWithTokenRequest] {
  ...
  /** filter component
    ...
    * @return
    */
  override def filter[A](request: LogTraceWithTokenRequest[A]) = Future.successful {
    request.trace match {
